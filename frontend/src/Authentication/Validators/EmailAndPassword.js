...
export async function handleEmail(email, setValidEmail) {
    try {
      if (isValidEmailFormat(email)) {
        ...
        if (isValidEmail) {
          setValidEmail(true);
          return <span className='text-green-500 text-xs'>&#x2713;Email is valid and available</span>
        } else {
          setValidEmail(false);
          return <span className='text-red-500 text-xs'>&#x274C;Email already exists</span>
        }
      } else {
        setValidEmail(false);
        return <span className='text-red-500 text-xs'>&#x274C;Invalid email format</span>
      }
    } catch (error) {
      setValidEmail(false);
      return <span className='text-red-500 text-xs'>&#x274C;An error occurred while checking the email</span>
    }
  }
 ...
 export function handlePassword(password, setValidPass) {
    ...
    if (isValid) {
      setValidPass(true);
      return <span className='text-green-500 text-xs'>&#x2713;Valid Password</span>
    } else {
      setValidPass(false);
  ...
  export function handleConfirmPass(password, confirmPassword, setConfirmPass) {
    ...
    if(confirm){
      setConfirmPass(true);
      return <span className='text-green-500 text-xs'>&#x2713;Passwords match</span>
    }
    else{
      setConfirmPass(false);
    return <span className='text-red-500 text-xs'>&#x274C;Passwords doesn't match</span>
    }
  }
